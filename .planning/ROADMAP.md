# Project Roadmap

## Current Milestone: v1.1 Security & Engagement

### Phase 6: Auth & User Accounts

**Goal:** Secure login, profile sync, and premium tier gating.
**Depends on:** Phase 5 (v1.0)
**Status:** Completed
**Completed:** 2026-02-02
**Plans:** 3 plans

Plans:
- [x] 06-01-PLAN.md — Auth Foundation & Session Management
- [x] 06-02-PLAN.md — Authentication UI & React 19 Provider
- [x] 06-03-PLAN.md — Cloud Sync & LocalStorage Migration

**Details:**
Implement secure user authentication (Firebase), user profiles, and prepare for premium features.

### Phase 7: Enhanced Compatibility Reports & Remedies

**Goal:** Detailed compatibility reports with side-by-side grids, remedies, and PDF export.
**Depends on:** Phase 6
**Status:** Completed
**Completed:** 2026-02-02
**Plans:** 3 plans

Plans:
- [x] 07-01-PLAN.md — Implement remedy mapping and deterministic scoring logic
- [x] 07-02-PLAN.md — Create comparison grids and remedy UI components
- [x] 07-03-PLAN.md — Implement dual-profile PDF generation and export

**Details:**
Show more details in compatibility report like Lo Shu grid of both persons, export functionality, detailed analysis with each number, how they complete each other, missing number significance, and remedies.

### Phase 8: Social Engagement

**Goal:** Viral sharing, daily cards, and push notifications to drive retention
**Depends on:** Phase 7
**Plans:** 0 plans

Plans:
- [ ] TBD (run /gsd:plan-phase 8 to break down)

**Details:**
Implement seamless sharing of daily forecasts and numerology charts, real-time updates, and push notifications to drive daily engagement.

---
