...
function exportToExcel() {
  ...
  if (typeof XLSX === "undefined") {
    alert(
      "Error: Excel library (SheetJS) failed to load. Please check your internet connection and ensure nothing is blocking the script."
    );
  ...
  const headers = [
    ...
    "Grid 4",
    "Grid 9",
    "Grid 2", // Corresponds to cells c1, c2, c3
    "Grid 3",
    "Grid 5",
    "Grid 7", // Corresponds to cells c4, c5, c6
    "Grid 8",
    "Grid 1",
    "Grid 6", // Corresponds to cells c7, c8, c9
  ];
  ...
  usersData.forEach((user) => {
    ...
    for (const headerName in gridCellContents) {
      if (headerToIndexMap.hasOwnProperty(headerName)) {
        ...
        const cellString = gridCellContents[headerName]
  ...
  try {
    ...
    alert(
      "An error occurred while generating the Excel file. Please check the console (F12) for details."
    );
  }
}
