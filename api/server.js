...
console.log(
  ...
  Object.keys(iplTeamsData).length > 0 ? `${Object.keys(iplTeamsData).length} teams loaded.` : "FAILED TO LOAD OR EMPTY"
)
  ...
  try {
    ...
    if (nameData) {
      addSection("Name Numerology Analysis", () => {
        ...
        doc
          .font(fonts.body)
          .text(`${destinyNumber} - ${nameDestinyMeanings[destinyNumber] || "No specific meaning found."}`)
        doc.moveDown(0.5)

        doc.font(fonts.bodyBold).text("Soul Urge Number: ", { continued: true })
        doc
          .font(fonts.body)
          .text(`${soulUrgeNumber} - ${nameSoulUrgeMeanings[soulUrgeNumber] || "No specific meaning found."}`)
        doc.moveDown(0.5)

        doc.font(fonts.bodyBold).text("Personality Number: ", { continued: true })
        doc
          .font(fonts.body)
          .text(`${personalityNumber} - ${namePersonalityMeanings[personalityNumber] || "No specific meaning found."}`)
      })
    } else if (name) {
      addSection("Name Numerology Analysis", () => {
...
app.get("/api/report/compatibility/pdf", async (req, res) => {
  ...
  try {
    ...
    console.log(`[/api/report/compatibility/pdf] Grid Analysis P1 (${name1}):`, person1GridAnalysis) // <-- Add console log
    console.log(`[/api/report/compatibility/pdf] Grid Analysis P2 (${name2}):`, person2GridAnalysis) // <-- Add console log
    ...
    const addPersonSection = (name, dob, gender, data) => {
      addSection(`${name}'s Details`, () => {
        ...
        doc
          .font(fonts.bodyBold)
          .text(`Moolank-Bhagyank (${data.moolank}-${data.bhagyank}):`, textStartX, doc.y, { width: textWidth })
        if (mbRelation) {
          doc
            .font(fonts.body)
            .text(`  ${mbRelation.indication || "N/A"} (Rating: ${mbRelation.rating || "N/A"})`, { width: textWidth })
        } else {
