...
function CompatibilityChecker() {
  ...
  const handleCalculateCompatibility = async () => {
    ...
    if (data1 && data2) {
      ...
      const person1MBRelation = moolankBhagyankRelations[m1]?.[b1] || {
        rating: "N/A",
        indication: "No specific data",
      };
      const person2MBRelation = moolankBhagyankRelations[m2]?.[b2] || {
        rating: "N/A",
        indication: "No specific data",
      };
      ...
      setCompatibilityDetails({
        ...
        breakdown: [
          // P1 -> P2
          {
            p1: `Moolank (${m1})`,
            p2: `Moolank (${m2})`,
            relation: relationM1M2,
            score: scoreM1M2,
          },
          {
            p1: `Bhagyank (${b1})`,
            p2: `Bhagyank (${b2})`,
            relation: relationB1B2,
            score: scoreB1B2,
          },
          {
            p1: `Moolank (${m1})`,
            p2: `Bhagyank (${b2})`,
            relation: relationM1B2,
            score: scoreM1B2,
          },
          {
            p1: `Bhagyank (${b1})`,
            p2: `Moolank (${m2})`,
            relation: relationB1M2,
            score: scoreB1M2,
          },
          // P2 -> P1
          {
            p1: `Moolank (${m2})`,
            p2: `Moolank (${m1})`,
            relation: relationM2M1,
            score: scoreM2M1,
          },
          {
            p1: `Bhagyank (${b2})`,
            p2: `Bhagyank (${b1})`,
            relation: relationB2B1,
            score: scoreB2B1,
          },
          {
            p1: `Moolank (${m2})`,
            p2: `Bhagyank (${b1})`,
            relation: relationM2B1,
            score: scoreM2B1,
          },
          {
            p1: `Bhagyank (${b2})`,
            p2: `Moolank (${m1})`,
            relation: relationB2M1,
            score: scoreB2M1,
          },
        ],
        person1: {
      ...
      setCompatibilityDetails({
        error:
          "Could not calculate compatibility due to invalid input or API error for one or both individuals.",
      });
    }
    // --- End Calculation ---
  ...
  return (
    <>
        ...
        <div id="person1Input" className="person-input-section">
          ...
          {!isLoading && person1Data && (
            <div className="grid-display">
              ...
              <p>
                Bhagyank: {person1Data.bhagyank}, Moolank: {person1Data.moolank}, Kua:{" "}
                {person1Data.kua}
              </p>
            </div>
          )}
          {/* Show error only after calculation attempt and if data is missing */}
          {calculationAttempted && !isLoading && !person1Data && (
            <p className="error-message">Could not load data for Person 1.</p>
          )}
        </div>
        ...
        <div id="person2Input" className="person-input-section">
          ...
          {!isLoading && person2Data && (
            <div className="grid-display">
              ...
              <p>
                Bhagyank: {person2Data.bhagyank}, Moolank: {person2Data.moolank}, Kua:{" "}
                {person2Data.kua}
              </p>
            </div>
          )}
          {/* Show error only after calculation attempt and if data is missing */}
          {calculationAttempted && !isLoading && !person2Data && (
            <p className="error-message">Could not load data for Person 2.</p>
          )}
        </div>
      </div>
          ...
          {compatibilityDetails.error ? (
            ...
            <>
              ...
              <div className="overall-score">
                ...
                {compatibilityDetails.score} / {compatibilityDetails.maxScore} (
                {compatibilityDetails.percentage}%)
                <br />
              ...
              <div className="individual-details-container">
                ...
                <div className="individual-details">
                  ...
                  <p>
                    <strong>Moolank:</strong> {compatibilityDetails.person1.moolank} (
                    {compatibilityDetails.person1.moolankMeaning})
                  </p>
                  <p>
                    <strong>Bhagyank:</strong> {compatibilityDetails.person1.bhagyank} (
                    {compatibilityDetails.person1.bhagyankMeaning})
                  </p>
                  <p>
                    <strong>
                      Moolank-Bhagyank Relation ({compatibilityDetails.person1.moolank}-
                      {compatibilityDetails.person1.bhagyank}):
                    </strong>{" "}
                    {compatibilityDetails.person1.mbRelation.indication} (Rating:{" "}
                    {compatibilityDetails.person1.mbRelation.rating})
                  </p>
                </div>
                {/* Person 2 Details */}
                <div className="individual-details">
                  ...
                  <p>
                    <strong>Moolank:</strong> {compatibilityDetails.person2.moolank} (
                    {compatibilityDetails.person2.moolankMeaning})
                  </p>
                  <p>
                    <strong>Bhagyank:</strong> {compatibilityDetails.person2.bhagyank} (
                    {compatibilityDetails.person2.bhagyankMeaning})
                  </p>
                  <p>
                    <strong>
                      Moolank-Bhagyank Relation ({compatibilityDetails.person2.moolank}-
                      {compatibilityDetails.person2.bhagyank}):
                    </strong>{" "}
                    {compatibilityDetails.person2.mbRelation.indication} (Rating:{" "}
                    {compatibilityDetails.person2.mbRelation.rating})
                  </p>
                </div>
              </div>
            </>
      ...
      >
        {/* Calculate Button is always visible */}
        <button onClick={handleCalculateCompatibility} disabled={isLoading}>
          {isLoading ? "Calculating..." : "Calculate Compatibility"}
        </button>
        {/* Download PDF Button - Show only after calculation attempt, enabled only when data is loaded and calculation succeeded */}
        {calculationAttempted && (
          <button
            ...
            disabled={
              ...
              !person1Data ||
              !person2Data ||
              (compatibilityDetails && compatibilityDetails.error)
            }
            title={
              !person1Data || !person2Data
                ? "Calculate compatibility first"
                : compatibilityDetails && compatibilityDetails.error
                  ? "Cannot download report due to calculation error"
                  : "Download PDF Report"
            }
          >
            Download PDF Report
